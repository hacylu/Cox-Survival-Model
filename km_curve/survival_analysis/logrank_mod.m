function info = logrank_mod(varargin)
...
%(xcg and ycg).
try
    [table1 table12 t1 T1 xcg1 ycg1 lambda1]=kmplotSomeCensored(x1,0.05,cflag,0);
    [table2 table22 t2 T2 xcg2 ycg2 lambda2]=kmplotSomeCensored(x2,0.05,cflag,0);
catch ME
...
S2=stairs(t2,T2,'b','LineWidth',3); %Kaplan-Meier curve for treatment 2
if ~isempty(table22)
...
xlabel('Time')
if isempty(S3)
    legend([S1 S2],{'Short Term','Long Term'},'FontSize', 11)
else
    legend([S1 S2 S3],{'Short Term','Long Term','Censored'}, 'FontSize', 11)
end
...
% hel = round(el1/2);
% if mod([el1],2) % if odd
%     mt1 = (t1( when1(hel) ) + t1( when1(hel +1) ) ) /2;
% else
%     mt1 = t1( when1(hel) );
% end
% 
% [what2,when2] = sort(T2);
% el2 = numel(what2);
% if mod([el2],2) % if even
%     mt2 = (t2( (when2(hel) ) + t2(when2(hel +1)) ) )/2;
% else
%     mt2 = t2( when2(hel) );
% end
