function info = logrankMulti(varargin)
...
[x1, x2, x3, alpha, cflag] = deal(default.values{:});
if ~all(isfinite(x1(:))) || ~all(isnumeric(x1(:))) || ~all(isfinite(x2(:))) || ~all(isnumeric(x2(:))) || ~all(isfinite(x3(:))) || ~all(isnumeric(x3(:)))
    error('Warning: all X1, X2, X3 values must be numeric and finite')
end
if isvector(x1) 
    x1(:,2)=0;
else
    ...
    if ~all(x1(:,2)==-1 | x1(:,2)==0 | x1(:,2)==1)
        error('Warning: all X1(:,2) values must be -1, 0, or 1')
    end
end
if isvector(x2) 
    x2(:,2)=0;
    ...
    if ~all(x2(:,2)==-1 | x2(:,2)==0 | x2(:,2)==1)
        error('Warning: all X2(:,2) values must be -1, 0, or 1')
    end
end
if isvector(x3)
    x3(:,2)=0;
else
    if ~isequal(size(x3,2),2)
        error('LOGRANK requires Nx2 matrix data.');
    end
    if ~all(x3(:,2)==-1 | x3(:,2)==0 | x3(:,2)==1)
        error('Warning: all X3(:,2) values must be -1, 0, or 1')
    end
end
if nu>=3
    if isempty(alpha)
...
%recall modified KMPLOT function to construct tables of data (table1,
%table2, and table3), tables of censored data table12 and table22),
%Kaplan-Meier varaiables (t1, t2, t3, T1, T2, T3) and Kaplan-Meier
%graphical data for censored data (xcg and ycg).
try
    [table1, table12, t1, T1, xcg1, ycg1, lambda1]=kmplotMultiClassCensored(x1,0.05,cflag,0);
    [table2, table22, t2, T2, xcg2, ycg2, lambda2]=kmplotMultiClassCensored(x2,0.05,cflag,0);
    [table3, table32, t3, T3, xcg3, ycg3, lambda3]=kmplotMultiClassCensored(x3,0.05,cflag,0);
catch ME
...
hold on
S1=stairs(t1,T1,'r','LineWidth',3); %Kaplan-Meier curve for treatment 1
S2=stairs(t2,T2,'g','LineWidth',3); %Kaplan-Meier curve for treatment 2
S3=stairs(t3,T3,'b','LineWidth',3); %Kaplan-Meier curve for treatment 3
if ~isempty(table32)
    S4=plot(xcg3,ycg3,'k+'); %Censored data for treatment 3 (if there are)
end
if ~isempty(table22)
    S4=plot(xcg2,ycg2,'k+'); %Censored data for treatment 2 (if there are)
end
if ~isempty(table12)
    S4=plot(xcg1,ycg1,'k+'); %Censored data for treatment 1 (if there are)
else
    S4=[];
end
...
xlabel('Time')
if isempty(S4)
    legend([S1 S2 S3],{'Short Term','Mid Term','Long Term'},'FontSize', 11)
else
    legend([S1 S2 S3 S4],{'Short Term','Mid Term','Long Term','Censored'}, 'FontSize', 11)
end

info = [];
% COMMENTED OUT TO REMOVE BUGS!!!!!!
% for i = 1:2
%     if i == 2
%         T1 = T2; t1 = t2;
%     end
...
% %Merge the first columns of Table1, Table2, and Table3 (time intervals)
% %and pick-up unique values
% A=unique([table1(:,1);table2(:,1);table3(:,1)]);
...
% %Put in the columns 6 and 7 and in the proper rows the deaths and alive
% %taken from table3 columns 2 and 3
% [~, ia ib]=intersect(table3(:,1),A);
% table(ib,6:7)=table2(ia,2:3);
% %remove the rows where there arent't deaths in all three treatments
% table((table(:,2)==0 & table(:,4)==0 & table(:,6)==0),:)=[];
...
% info.p = p;
% info.alpha = alpha;
% info.hazardrate1 = lambda1;
% info.hazardrate2 = lambda2;
% info.hazardratio = lambda1/lambda2;
%info.mediansurvivaltime = mt1;
